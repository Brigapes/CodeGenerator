...
void Code::RandomFullLoop() {
	...
	if (FiftyFifty()) {
		...
		PrintSpace(); Sleep(70); std::cout << "         ";
		std::cout << RandomFunction() << "(" << RandomProWord() << ");" << std::endl;
		PrintSpace(); Sleep(150); std::cout << "         ";
		std::cout << RandomProWord() << "++;" << std::endl;
		PrintSpace(); Sleep(150);
		std::cout << "}" << std::endl;
	}
	else {
...
void Code::PrintRandomClassVars() {
	...
	PrintSpace(); Sleep(10);
	for (int i = 0; i < (RandomRange(0, MaxClassVars)); i++) {	Sleep(180); 
	std::cout << RandomType() << " " << RandomProWord() << " = " << RandomValue() << std::endl;
		PrintSpace();}
	std::cout << "" << std::endl; PrintSpace();
	std::cout << RandomAccesor() << std::endl;
	PrintSpace(); Sleep(10);
	for (int i = 0; i < (RandomRange(0, MaxClassVars+4)); i++) {Sleep(180); 
	std::cout << RandomType() << " " << RandomFunction() << "( " << RandomType() << " );" << std::endl;
		PrintSpace();}
	
}

std::string Code::RandomType() {Sleep(50);
...
std::string Code::GetSetRandomClassName() {Sleep(30);
	std::string crr= RClassName[RandomRange(0, RClassName.size())];
	CurrentClass = crr; return crr;}

void Code::ClassStuff() {Sleep(30);
	StartLayer();
	PrintSpace();

	std::cout << " class " << GetSetRandomClassName() << std::endl; Sleep(30);
	PrintSpace(); Sleep(50);
...
std::string Code::RandomFunction() {Sleep(80);
...
void Code::PrintStart() {
	std::cout << "#include <iostream>" << std::endl; Sleep(80);
	std::cout << "#include <vector>" << std::endl; Sleep(80);
	std::cout << "#include <string>" << std::endl; Sleep(80);
	std::cout << "#include \"boost.h\"" << std::endl; Sleep(140);
	std::cout << "#include \"CustomFunctions.h\"" << std::endl; Sleep(140);
}
